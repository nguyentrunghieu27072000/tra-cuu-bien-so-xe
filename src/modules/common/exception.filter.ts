...
import { ApiGeneralError } from './classes/api-error.general';
...
const handleException = (exception: HttpException | Error) => {
  ...
  } else if (exception instanceof ApiGeneralError) {
    const apiGeneralResult = new ApiResult<any>();
    const error_code = exception.error_code;
    const error_message = exception.error_message;
    const details = exception.details;
    const help = exception.help;
    return apiGeneralResult.generalError(
      {
        error_code,
        error_message,
        details,
